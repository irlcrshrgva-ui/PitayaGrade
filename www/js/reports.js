...
const ReportsManager = {
  ...
  _getFilteredScans() {
    const fromInput = document.getElementById('reportFrom');
    const toInput = document.getElementById('reportTo');
    const from = fromInput ? new Date(fromInput.value + 'T00:00:00') : new Date(0);
    const to = toInput ? new Date(toInput.value + 'T23:59:59') : new Date();
  ...
  generateReport() {
    ...
    area.innerHTML = `
      <div class="report-preview" id="printableReport">
        ...
        <table>
          <tr><th>Date</th><th>Grade</th><th>Confidence</th><th>Disease</th><th>Size</th></tr>
          ...
          ${scans.length > 20 ? `<tr><td colspan="5" style="text-align:center;color:#999">... and ${scans.length - 20} more records</td></tr>` : ''}
        </table>
        ...
        <div style="margin-top:20px;padding-top:12px;border-top:1px solid #ddd;text-align:center">
          ...
          <p style="font-size:11px;color:#999;margin:2px 0 0">AI-Powered Dragon Fruit Quality Assessment System</p>
        </div>
      </div>
  ...
  exportCSV() {
    ...
    const headers = ['Date', 'Time', 'Grade', 'Grade Confidence', 'Disease', 'Disease Confidence', 'Size', 'Color Uniformity', 'Surface Condition', 'Processing Mode', 'Processing Time'];
